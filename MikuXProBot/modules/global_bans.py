...
def gban(update: Update, context: CallbackContext):
    ...
    log_message = (
        ...
        f"<b>• Inspector:</b> {mention_html(user.id, user.first_name)}\n"
        f"<b>• Target User:</b> {mention_html(user_chat.id, user_chat.first_name)}\n"
        f"<b>• Reason:</b> <code>{reason}</code>\n"
        f"<b>• Ban time:</b> <code>{current_time}</code>\n"
        f"<b>• Banned Chats:</b> <code>{gbanned_chats}</code>")
    ...
    if EVENT_LOGS:
        try:
            log = bot.send_message(
                EVENT_LOGS, log_message, reply_markup=InlineKeyboardMarkup(buttons),
                parse_mode=ParseMode.HTML,            
        )
        except BadRequest as excp:
            log = bot.send_message(
    ...
    if gban_time > 60:
        ...
        message.reply_message("Done! Gbanned.", parse_mode=ParseMode.HTML)
    else:
...
def ungban(update: Update, context: CallbackContext):
    ...
    log_message = (
        f"╒═══「 #UNGBANNED 」\n"
        f"<b>• Genarated from:</b> <code>{chat_origin}</code>\n"
        f"<b>• Enforcer:</b> {mention_html(user.id, user.first_name)}\n"
        f"<b>• Target User:</b> {mention_html(user_chat.id, user_chat.first_name)}\n"
        f"<b>• Target User ID:</b> <code>{user_chat.id}</code>\n"
        f"<b>• Event Stamp:</b> <code>{current_time}</code>\n"
        f"<b>• Chats Affected:</b> <code>{ungbanned_chats}</code>")
    ...
    if ungban_time > 60:
        ...
        message.reply_message(
            UNGBAN_GIF, f"Person has been un-gbanned. Took {ungban_time} min")
    else:
        message.reply_message(
            UNGBAN_GIF, f"Person has been un-gbanned. Took {ungban_time} sec")
...
def check_and_ban(update, user_id, should_message=True):
    ...
    [
            ...
            update.effective_message.reply_text(text, reply_markup=InlineKeyboardMarkup(buutons), parse_mode=ParseMode.HTML)
